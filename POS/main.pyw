...
def printVoucher():
    if CASH_RECEIVED >= PAYABLE_AMOUNT:
        ...
        class_name = str(customer_class.get())
        reference = str(customer_reference.get())
        global today
        str_today = str(today)
        now = datetime.now()
        ...
        c.execute("INSERT INTO orders VALUES (:customer_name, :customer_phone, :class, :reference, :quantity, :subtotal, :discount, :payable_amount, :gift, :cash_received, :cash_return, :date, :time, :product_ids, :product_names)", 
            {
                'customer_name': cust_name,
                'customer_phone': cust_phone,
                'class': class_name,
...
def win_pdf_print(file_to_print):
    if file_to_print:
...
def todays_sale():
    ...
    # making csv file 
    header = ["customer_name", "customer_phone", "class", "reference", "quantity", "subtotal", "discount", "payable_amount", "gift", "cash_received", "cash_return", "date", "time", "product_ids", "product_names"]
...
customer_class_label = Label(cust_frame, text="Class", pady=10)
customer_class_label.grid(row=1, column=0)
customer_class = Entry(cust_frame, width=51)
customer_class.grid(row=1, column=1)

customer_reference_label = Label(cust_frame, text="Reference", pady=10)
customer_reference_label.grid(row=1, column=2)
customer_reference = Entry(cust_frame, width=31)
customer_reference.grid(row=1, column=3)
