...
if not os.path.exists('./.db/products.db'):
    ...
    products = [("12001 - ONNOROKOM BIGGANBAKSHO (ALOR JHALAK)",	"10000001",	925),
        ("12021 - ONNOROKOM SCIENCE BOX (COLOR OF LIGHT)",	"40000001",	0),
        ...
        ("12012 - MAJAR PERISCOPE",	"14000001",	0),
...
def printVoucher():
    ...
    other_products = [("ONNOROKOM BIGGANBAKSHO (ALOR JHALAK)[BDT 925]"),
        ("ONNOROKOM SCIENCE BOX (COLOR OF LIGHT)[BDT 0]"),
    ...
    c.execute("INSERT INTO orders VALUES (:customer_name, :customer_phone, :quantity, :subtotal, :discount, :cash_received, :cash_return, :date, :time, :product_ids, :product_names)", 
        {
            'customer_name': cust_name,
            'customer_phone': cust_phone,
...
def open_pdf():
    base_dir = os.getcwd()
    path = base_dir+'\.template\pdf_generated.pdf'
    os.system(path)
...
def todays_sale():
    ...
    # making csv file 
    header = ["customer_name", "customer_phone", "quantity", "subtotal", "discount", "cash_received", "cash_return", "date", "time", "product_ids", "product_names"]
    ...
    # with open(path+'/todays_sale.csv', 'w', encoding='UTF8') as f:
    #     writer = csv.writer(f)
    #     # write the header
...
def one_month_sale():
    ...
    # create cursor 
    c = conn.cursor()
    lookup_date_begin = str(today)
    lookup_date_end = str(today + dt.timedelta(days=30))
